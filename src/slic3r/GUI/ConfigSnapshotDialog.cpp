...
static wxString generate_html_row(const Config::Snapshot &snapshot, bool row_even, bool snapshot_active)
{
    ...
    text += "</b></font><br>";
    // End of row header.
    text += _(L("MXLabSlicer version")) + ": " + snapshot.slic3r_version_captured.to_string() + "<br>";
    ...
    for (const Config::Snapshot::VendorConfig &vc : snapshot.vendor_configs) {
        text += _(L("vendor")) + ": " + vc.name +", " + _(L("version")) + ": " + vc.version.config_version.to_string() + 
				", " + _(L("min MXLabSlicer version")) + ": " + vc.version.min_slic3r_version.to_string();
        if (vc.version.max_slic3r_version != Semver::inf())
            text += ", " + _(L("max MXLabSlicer version")) + ": " + vc.version.max_slic3r_version.to_string();
        text += "<br>";
