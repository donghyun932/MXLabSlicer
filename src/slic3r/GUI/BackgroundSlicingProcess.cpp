    ...
    */
	...
	if (this->set_step_started(bspsGCodeFinalize)) {
	    if (! m_export_path.empty()) {
		    ...
		    if (copy_file_custom(m_temp_output_path, export_path, GUI::RemovableDriveManager::get_instance().is_path_on_removable_drive(export_path), m_fff_print->config().option<ConfigOptionBool>("shield_gas_applied")->value, m_fff_print->config().option<ConfigOptionFloat>("dwell_time")->value, m_fff_print->config().option<ConfigOptionFloat>("traverse_speed")->value, m_fff_print->config().option<ConfigOptionString>("powder_feeder_define")->value) != 0)
