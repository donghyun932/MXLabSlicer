    ...
    if (!m_gl_initialized)
    {
        ...
        if (! s_gl_info.is_version_greater_or_equal_to(2, 0)) {
        	...
        	wxString message = wxString::Format(
        		_(L("MXLabSlicer requires OpenGL 2.0 capable graphics driver to run correctly, \n"
...
#ifdef _WIN32
        	message += "\n";
        	message += _(L("As a workaround, you may run MXLabSlicer with a software rendered 3D graphics by running mxlab-slicer.exe with the --sw_renderer parameter."));
#endif
        	wxMessageBox(message, wxString("MXLabSlicer - ") + _(L("Unsupported OpenGL version")), wxOK | wxICON_ERROR);
        }
    }
}
