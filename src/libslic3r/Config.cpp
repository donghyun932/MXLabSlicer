...
void ConfigBase::load_from_gcode_file(const std::string &file)
{
	...
	boost::nowide::ifstream ifs(file);
	{
        ...
        const char mxlabslicer_gcode_header[] = "; generated by MXLabSlicer ";
		std::string firstline;
		std::getline(ifs, firstline);
		if (strncmp(slic3r_gcode_header, firstline.c_str(), strlen(slic3r_gcode_header)) != 0 &&
            strncmp(mxlabslicer_gcode_header, firstline.c_str(), strlen(mxlabslicer_gcode_header)) != 0)
			throw std::runtime_error("Not a MXLabSlicer / Slic3r PE generated g-code.");
	}
    ifs.seekg(0, ifs.end);
