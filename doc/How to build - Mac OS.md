...
# Building MXLabSlicer on Mac OS

To build MXLabSlicer on Mac OS, you will need the following software:
...
MXLabSlicer comes with a set of CMake scripts to build its dependencies, it lives in the `deps` directory.
Open a terminal window and navigate to MXLabSlicer sources directory and then to `deps`.
...
### Building MXLabSlicer

If dependencies are built without errors, you can proceed to build MXLabSlicer itself.
Go back to top level MXLabSlicer sources directory and use these commands:
...
The CMake command above prepares MXLabSlicer for building from the command line.
...
and then open the `MXLabSlicer.xcodeproj` file.
This should open up XCode where you can perform build using the GUI or perform other tasks.

### Note on Mac OS X SDKs

By default MXLabSlicer builds against whichever SDK is the default on the current system.
...
on both the dependencies bundle as well as MXLabSlicer itself.

Official Mac MXLabSlicer builds are currently built against SDK 10.9 to ensure compatibility with older Macs.
